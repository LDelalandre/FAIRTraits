...
## 1) Before adding filed campaign ID ####
...
core %>% 
  ...
  filter(Treatment == "Ambient") %>% 
  filter(Year == 2012) %>% 
  select(Code_Sp, day_of_year) %>% unique() 
...
## 2) After having added field campaign ID ####

core2 %>% 
  filter(verbatimOccurrenceID_field_campaign == "problem") %>%
  select(verbatimTraitName,Site,Year) %>% unique()

core2 %>% filter(verbatimTraitName == "SdDM_msd") %>% View
